...
namespace api_constants
{
    ...
    const uint16_t addressBodyLength = wallet_config::standardAddressLength 
                                     - wallet_config::addressPrefix.length();

    /* This is the equivalent of TRTL[a-zA-Z0-9]{95} but working for all coins */
    const std::string addressRegex
        = std::string(wallet_config::addressPrefix) + "[a-zA-Z0-9]{" 
